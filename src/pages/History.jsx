      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mx-4 mb-5 bg-surface rounded-[20px] p-5 shadow-card border border-border-warm">
          <div className="flex items-center justify-between mb-4">
            <button onClick={prevMonth}><ChevronLeft size={22} className="text-accent" /></button>
            <h2 className="text-lg font-bold text-ink">{monthNames[month]} {year}</h2>
            <button onClick={nextMonth}><ChevronRight size={22} className="text-accent" /></button>
          </div>
          <div className="grid grid-cols-7 text-center mb-2">
            {["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map((d) => (
              <span key={d} className="text-xs font-semibold text-accent">{d}</span>
            ))}
          </div>
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <button key={day} onClick={() => setSelectedDay(day)}
                  className={`py-1.5 rounded-[10px] text-sm font-medium relative flex flex-col items-center transition-colors ${
                    isSelected ? "bg-accent-tint border-2 border-accent text-accent" : isToday ? "text-accent font-bold" : "text-ink"
                  }`}>
                  {day}
                  {hasShot && <div className="w-1.5 h-1.5 bg-success rounded-full mt-0.5" />}
                </button>
              );
            })}
          </div>
        ...
        {selectedDay && selectedShots.length === 0 && (
          <div className="px-4 mb-4">
            <div className="bg-surface rounded-[20px] p-4 text-center border border-border-warm text-ink-tertiary text-sm">
          ...
          <button onClick={() => setShowSideEffects(true)}
            ...
            {sideEffects ? (
              ...
              <div className="bg-accent-tint rounded-[14px] p-3 flex items-center gap-2">
                ...
                <p className="text-sm text-accent">Tap to add side effects for this day.</p>
              </div>
            )}
          </button>
        )}
