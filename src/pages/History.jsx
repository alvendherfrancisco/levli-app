      ...
      <div className="max-w-3xl mx-auto pt-4 pb-4">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              <button key={day} onClick={() => setSelectedDay(day)}
                className={`py-1.5 rounded-lg text-sm font-medium relative flex flex-col items-center ${
                  ...
                  {day}
                  {hasShot && <div className="w-2 h-2 bg-teal-500 rounded-full mt-0.5" style={{boxShadow:"0 0 6px 2px rgba(74,222,128,0.5)"}} />}
                </button>
              );
            })}
          </div>
        ...
        {selectedDayKey && (
          <button onClick={() => setShowSideEffects(true)}
            className="relative overflow-hidden min-h-[260px] mx-4 mb-4 bg-white dark:bg-gray-900 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-800 w-[calc(100%-2rem)] text-left">
