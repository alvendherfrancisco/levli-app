    ...
    <div className="min-h-screen w-full">
      ...
      <div className="max-w-3xl mx-auto pt-4 pb-6">
        ...
        <button
          ...
          className="relative overflow-hidden mx-3 mb-4 rounded-2xl p-4 shadow-sm border border-black/5 w-[calc(100%-1.5rem)] text-left"
        >
          <ScatteredFaces color="indigo" count={5} opacity={0.4} />
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-white/40 backdrop-blur-sm">
                <Wind size={16} className="text-indigo-700" />
              </div>
              <span className="font-semibold text-indigo-900">Side effects</span>
            </div>
                ...
                {dayAdverseEvents.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-2">
                    {dayAdverseEvents.map((e) => (
                      <span key={e.id} className="text-xs bg-white/70 text-indigo-700 rounded-full px-2 py-0.5 border border-indigo-300/40">
                        {e.symptom} <span className="opacity-60">({e.severity})</span>
                      </span>
                    ))}
                  </div>
                )}
                {sideEffects && <p className="text-sm text-indigo-900">{sideEffects}</p>}
              </div>
            ) : (
              <div className="bg-white/50 backdrop-blur-sm rounded-xl p-3 flex items-center gap-2">
                <Info size={16} className="text-indigo-700 flex-shrink-0" />
                <p className="text-sm text-indigo-800">Tap to add side effects.</p>
              </div>
            )}
          </div>
        </button>
