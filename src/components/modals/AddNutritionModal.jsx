...
export default function AddNutritionModal({ open, onClose }) {
  const [values, setValues] = useState({ calories: "0.0", protein: "0.0", water: "0.0", fiber: "0.0", carbs: "0.0" });
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative bg-white rounded-t-3xl w-full max-w-lg animate-in slide-in-from-bottom">
        ...
        <div className="flex gap-3 px-5 pb-8 pt-2">
          <button
            onClick={() => setValues({ calories: "0.0", protein: "0.0", water: "0.0", fiber: "0.0", carbs: "0.0" })}
            className="flex-1 py-3.5 bg-gray-200 text-gray-600 rounded-xl font-semibold flex items-center justify-center gap-2"
          >
            <RotateCcw size={16} /> Reset
          </button>
          <button className="flex-1 py-3.5 bg-blue-600 text-white rounded-xl font-semibold flex items-center justify-center gap-2">
            <Save size={16} /> Save
          </button>
        </div>
      </div>
