...
export default function QuizScreen({ question, subtitle, options, multiSelect, preSelected, onAnswerChange }) {
  ...
  return (
    <div className="flex-1">
      <h1 className="text-2xl sm:text-3xl font-bold leading-tight mb-2">{question}</h1>
        ...
        {options.map((opt) => {
          ...
          return (
            <button
              key={opt}
              onClick={() => toggle(opt)}
              className={`w-full flex items-center justify-between px-4 py-3.5 sm:py-4 rounded-xl border transition-all text-left ${
                isSelected
                  ? "border-teal-500 bg-teal-500/10 text-teal-400"
                  : "border-gray-700 bg-gray-900 text-white"
              }`}
            >
              <span className="text-sm sm:text-[15px] pr-3">{opt}</span>
              <div
